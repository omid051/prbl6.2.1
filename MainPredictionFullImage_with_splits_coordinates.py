...
def main() -> None:
    ...
    for res in most_common_results:
        ...
        # print("Number of tiles:", indices_plus_1)  # ✅ COMMENTED (print)

        # ✅ NEW OUTPUT FORMAT:
        # coordinates:x=123,y=2 , x=... , ...
        parts = []
        for tile_num in indices_plus_1:
            if tile_num not in TILE_BBOXES:
                continue

            x1, y1, x2, y2 = TILE_BBOXES[tile_num]

            # اگر top-left می‌خواهی: cx=x1 و cy=y1
            cx = (x1 + x2) // 2
            cy = (y1 + y2) // 2
