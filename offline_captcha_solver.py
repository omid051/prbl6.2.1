...
def _sanitize_layer_config(layer: dict):
    ...
    if class_name == "LeakyReLU":
        if "negative_slope" in cfg and "alpha" not in cfg:
            cfg["alpha"] = cfg.pop("negative_slope")


def load_h5_fix(h5_path: str):
    with h5py.File(h5_path, "r") as f:
        mc = f.attrs.get("model_config")
        if mc is None:
            raise ValueError("این فایل .h5 معماری مدل را ندارد (weights-only است).")
        if isinstance(mc, bytes):
            mc = mc.decode("utf-8")

    cfg = json.loads(mc)
    for layer in cfg.get("config", {}).get("layers", []):
        _sanitize_layer_config(layer)

    model = model_from_json(
        json.dumps(cfg),
        custom_objects={"LeakyReLU": LeakyReLU, "Rescaling": Rescaling},
    )
    model.load_weights(h5_path)
    return model


model = load_h5_fix("bls_captcha_model.h5")
print("Loaded ✅")

# model.summary()

INPUT_PATH = r"C:\Users\Omid\Desktop\test\captcha.jpg"
...
# ✅ NEW: حذف آرتیفکت‌های لبه با crop + repeat (روی تصاویر train_img که قبلاً داخل Converted_to_train_image ذخیره می‌شد)
CROP_REPEAT_TOP_PX = 3
CROP_REPEAT_BOTTOM_PX = 3
CROP_REPEAT_LEFT_PX = 3
CROP_REPEAT_RIGHT_PX = 3
...
# ✅ NEW: ذخیره‌ی مختصات tile ها روی تصویر اصلی (INPUT_PATH)
# tile_num -> (x1, y1, x2, y2)  در همان مختصات پیکسلی تصویر مرجع
TILE_BBOXES: dict[int, tuple[int, int, int, int]] = {}
...
def extract_9_tiles_full_image(
    ...
    for i, (x, y, w, h) in enumerate(boxes, start=1):
        ...
        # ✅ NEW: ذخیره‌ی مختصات همین tile روی تصویر مرجع
        TILE_BBOXES[i] = (x1, y1, x2, y2)
...
def crop_and_repeat_edges(
    ...
    """
    از هر 4 طرف به اندازه‌ی مشخص کراپ می‌کند و برای حفظ اندازه،
    همان مقدار را با تکرار لبه‌ی جدید (بعد از کراپ) به همان طرف اضافه می‌کند.

    ورودی: img با شکل (H,W) یا (H,W,C)
    خروجی: هم‌اندازه‌ی ورودی
    """
    if min(top_px, bottom_px, left_px, right_px) < 0:
        raise ValueError("crop sizes must be >= 0")

    H, W = img.shape[:2]
    if top_px + bottom_px >= H:
        raise ValueError(f"top+bottom must be < H. Got {top_px}+{bottom_px} >= {H}")
    if left_px + right_px >= W:
        raise ValueError(f"left+right must be < W. Got {left_px}+{right_px} >= {W}")

    # 1) crop
    if img.ndim == 2:
        out = img[top_px:H - bottom_px, left_px:W - right_px]
    elif img.ndim == 3:
        out = img[top_px:H - bottom_px, left_px:W - right_px, :]
    else:
        raise ValueError(f"Expected (H,W) or (H,W,C), got {img.shape}")

    # 2) restore height by repeating top/bottom edge
    if top_px > 0:
        top_strip = out[:1, ...]  # یک ردیف از بالای جدید
        top_pad = np.repeat(top_strip, repeats=top_px, axis=0)
        out = np.concatenate([top_pad, out], axis=0)

    if bottom_px > 0:
        bottom_strip = out[-1:, ...]  # یک ردیف از پایینِ جدید
        bottom_pad = np.repeat(bottom_strip, repeats=bottom_px, axis=0)
        out = np.concatenate([out, bottom_pad], axis=0)

    # 3) restore width by repeating left/right edge
    if left_px > 0:
        left_strip = out[:, :1, ...]  # یک ستون از چپِ جدید
        left_pad = np.repeat(left_strip, repeats=left_px, axis=1)
        out = np.concatenate([left_pad, out], axis=1)

    if right_px > 0:
        right_strip = out[:, -1:, ...]  # یک ستون از راستِ جدید
        right_pad = np.repeat(right_strip, repeats=right_px, axis=1)
        out = np.concatenate([out, right_pad], axis=1)

    # sanity
    if out.shape[:2] != (H, W):
        raise RuntimeError(f"Shape mismatch: expected {(H, W)} got {out.shape[:2]}")
...
def convert_tiles_to_train_size(
    ...
    pad: int = 0,
    top_px: int = CROP_REPEAT_TOP_PX,
    bottom_px: int = CROP_REPEAT_BOTTOM_PX,
    left_px: int = CROP_REPEAT_LEFT_PX,
    right_px: int = CROP_REPEAT_RIGHT_PX,
) -> None:
    ...
    for in_path in tile_paths:
        ...
        # ✅ NEW: crop + repeat روی لبه‌ها (قبل از resize) برای حذف خط‌های ریز لبه‌ای
        cropped_bgr = crop_and_repeat_edges(
            cropped_bgr,
            top_px=top_px,
            bottom_px=bottom_px,
            left_px=left_px,
...
def extract_9_tiles_full_image_inmemory(
    input_path: str | os.PathLike,
    roi_height: int = ROI_HEIGHT,
    area_threshold: int = AREA_THRESHOLD,
    pad: int = TILE_PAD,
) -> tuple[list[np.ndarray], dict[int, tuple[int, int, int, int]]]:
    img = cv2.imread(str(input_path))
    if img is None:
        raise FileNotFoundError(f"Could not read: {input_path}")
    ...
    boxes = sorted(boxes, key=lambda b: (b[1], b[0]))
    if len(boxes) != 9:
        raise RuntimeError(f"Expected 9 boxes, but found {len(boxes)}.")
...
def tile_to_train_image(tile_bgr: np.ndarray) -> np.ndarray:
    bgra = keep_only_inside_of_frame(tile_bgr, border_px=BORDER_PX)
    cropped = crop_to_alpha(bgra, pad=0)
    cropped_bgr = cropped[..., :3]

    # ✅ اعمال همان ایده crop+repeat روی خروجی train image pipeline (قبل از resize)
    cropped_bgr = crop_and_repeat_edges(
        cropped_bgr,
        top_px=CROP_REPEAT_TOP_PX,
        bottom_px=CROP_REPEAT_BOTTOM_PX,
        left_px=CROP_REPEAT_LEFT_PX,
        right_px=CROP_REPEAT_RIGHT_PX,
    )

    resized = cv2.resize(cropped_bgr, (TRAIN_W, TRAIN_H), interpolation=cv2.INTER_AREA)
    return resized


def build_centered_mask_from_train_img(train_bgr: np.ndarray) -> np.ndarray:
    mask = segment_digits_kmeans(train_bgr, K=2, median_ksize=3)
    mask_noline, _ = remove_horizontal_lines(mask, line_frac=0.35)
    final_mask = crop_center_resize(mask_noline, out_w=TRAIN_W, out_h=TRAIN_H, pad=MASK_PAD)
    return final_mask
...
def main() -> None:
    # Step 1: extract tiles + bboxes (in-memory)
    tiles, bboxes = extract_9_tiles_full_image_inmemory(INPUT_PATH)

    # Step 2-5: تبدیل tile به train_img و ساخت mask (in-memory)
    masks: list[np.ndarray] = []
    for t in tiles:
        train_img = tile_to_train_image(t)
        m = build_centered_mask_from_train_img(train_img)
        masks.append(m)

    # Step 6-8: predict (in-memory)
    pred_strings: dict[int, str] = {}
    for idx in range(9):
        img_raw = masks[idx]  # (80x150) grayscale
    ...
    for res in most_common_results:
        indices_plus_1 = [i + 1 for i in result_to_indices[res]]

        parts = []
        for tile_num in indices_plus_1:
            x1, y1, x2, y2 = bboxes[tile_num]
            cx = (x1 + x2) // 2
            cy = (y1 + y2) // 2
            parts.append(f"x={cx},y={cy}")

        print("Number of tiles:", indices_plus_1)
        print("coordinates:" + ";".join(parts))
